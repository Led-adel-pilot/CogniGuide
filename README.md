 ...
 *   **PDF Generation:** Users can export to PDF from `components/MindMapModal.tsx`. The download flow now posts serialized markup and styles to `/api/mindmap/pdf`, where a headless Chromium instance (via `@sparticuz/chromium` + `puppeteer-core`) prints the DOM as a vector PDF. The exported file keeps connectors crisp and text selectable instead of rasterizing the map.
